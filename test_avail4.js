const { Business, Employee, Service, Schedule, SpecialSchedule } = require('./src/models');
...
async function test() {
  try {
    ...
    const regSched = await Schedule.create({
    ...
    const specSched = await SpecialSchedule.create({
      businessId: business.id,
      employeeId: employee.id,
      type: 'blocked',
    ...
    console.log('Reg Sched:', regSched.toJSON());
    console.log('Spec Sched:', specSched.toJSON());
    console.log('Date:', dateStr, 'DayOfWeek:', dayOfWeek);
